...
(defvar keywiz-minor-mode-maps nil)
  ...
  (let ((score 0)
	...
	(when (and (commandp sym)
	  ...
	  (let ((keys (apply 'nconc (mapcar
					 ...
					 (list key)))
				     (where-is-internal sym overriding-local-map nil nil)))))
      ...
      (setq keywiz-global-map (copy-keymap (current-global-map))
	    keywiz-local-map (copy-keymap (current-local-map))
	    keywiz-minor-mode-maps (current-minor-mode-maps)))
    (setq start-time (keywiz-float-time))
    
    (switch-to-buffer (get-buffer-create "*keywiz*"))
    (use-global-map keywiz-global-map)

    (dolist (l keywiz-minor-mode-maps)
      (setq keywiz-local-map (append l keywiz-local-map)))
;;     (push keywiz-minor-mode-maps keywiz-local-map)
